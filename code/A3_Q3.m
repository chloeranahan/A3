...
if b1 == 1
    bb1 = rectangle('Position',[0.8E-7,0,0.4E-7,0.4E-7]); %box1 position (bottom)
    tb1 = rectangle('Position',[0.8E-7,0.6E-7,0.4E-7,0.4E-7]); %box1 position (top)
...
for i = 1:nx
    for j = 1:ny
        ...
        if l > 0.8E-7 && l < 1.2E-7 && (w > 0.6E-7 || w < 0.4E-7) && b1 == 1
            C(i,j) = CondBox;
...
for i = 1:tstop
    ...
    if b1 == 1
        in_bot_box = Px > 0.8E-7 & Px < 1.2E-7 & Py < 0.4E-7; %in bottom box
    elseif b2 == 1
    ...
    if b1 == 1
        in_top_box = Px > 0.8E-7 & Px < 1.2E-7 & Py > 0.6E-7; %in top box
    elseif b2 == 1
...
Current = [0.5991;0.4063;0.1555];
T2 = table(BottleNeck,Current);
figure(4)
plot(T2.BottleNeck,T2.Current)
title('Average Current vs. Bottleneck Size')
